...
class SighashSpec extends FunSuite {
  test("SIGHASH_ANYONECANPAY lets you add inputs") {
    ...
    val previousTx = Seq(
      Transaction(version = 2, System.currentTimeMillis() / 1000, txIn = Nil, txOut = TxOut(42 millibtc, Script.pay2pkh(publicKeys(0))) :: Nil, lockTime = 0),
      Transaction(version = 2, System.currentTimeMillis() / 1000, txIn = Nil, txOut = TxOut(42 millibtc, Script.pay2pkh(publicKeys(1))) :: Nil, lockTime = 0)
    )

    // create a tx with no inputs
    val tx = Transaction(version = 2, System.currentTimeMillis() / 1000, txIn = Nil, txOut = TxOut(80 millibtc, Script.pay2wsh(Script.createMultiSigMofN(2, publicKeys))) :: Nil, lockTime = 0L)
  ...
  test("SIGHASH_ANYONECANPAY lets you add inputs (SEGWIT version") {
    ...
    val previousTx = Seq(
      Transaction(version = 2, System.currentTimeMillis() / 1000, txIn = Nil, txOut = TxOut(42 millibtc, Script.pay2wpkh(publicKeys(0))) :: Nil, lockTime = 0),
      Transaction(version = 2, System.currentTimeMillis() / 1000, txIn = Nil, txOut = TxOut(42 millibtc, Script.pay2wpkh(publicKeys(1))) :: Nil, lockTime = 0)
    )
